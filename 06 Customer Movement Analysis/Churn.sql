WITH cust_month AS (
    select distinct cust_code, date_trunc(shop_date) shop_month
    from `groovy-patrol-332408.Supermarket_Data.Supermarket_Table`
...
SELECT report_month
    ...
    ,COUNT(DISTINCT CASE WHEN status = '  ' THEN cust_code ELSE NULL END) AS reactivated_customers
...
FROM (
    SELECT cust_code, shop_month AS report_month, prev_month
        ,CASE 
            ...
            WHEN DATE_DIFF(shop_month, prev_month, MONTH) > 1 THEN 'Reactivate'
